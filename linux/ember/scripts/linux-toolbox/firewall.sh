...
ipMap() {
    local chain="$1"
    local ip="$2"
    iptables -A fw-fwd -d "$ip" -j "$chain"
}
...
allowUdp() {
    local chain="$1"
    local ports="$2"
    iptables -A "$chain" -p udp -m multiport --dports "$ports" -j ACCEPT
}

# Close a chain with a logging and drop rule
closeChain() {
    local chain="$1"
    iptables -A "$chain" -j LOG --log-prefix "[$chain-drop]: "
...
apply() {
    ...
    ## EDIT SUBNET AND PORTS
    # iptables -A INPUT -s 192.168.220.0/24 -j in-ok
    # IN CASE OF MULTIPLE SUBNETS
    # iptables -A INPUT -s 192.168.221.0/24 -j in-ok
    # iptables -A INPUT -p tcp -m multiport --dports 22,80,8080 -j in-ok
    ...
    ## Custom Router Rules Example
    # newChain box
    # allowTcp box 22,3389
    # allowUdp box 53
    # closeChain box

    # ipMap box 192.168.220.4
